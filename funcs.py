...
# TzUrl Shortner
Tz_api = '011fa9d202238764f8c23e300e6a8bb37b526271'

Tz_url = 'https://mdiskshortners.in/api?'
url_ptrn = r'https?://[^\s]+'
...
def addFooter(MSG, Footer):
    ...
    if RESULT == False:
        MSG.reply_text(
            """You have to /link your Mdiskshortner.link Or Mdisklink.link account first.\nHit 👉 /help to know more...""")
...
def short_urls(url_list, t_api=Tz_api):
    cnvt_urls = []
    for link in url_list:
        ...
        param = {'api': t_api, 'url': link}
        res = (rq.get(Tz_url, params=param))
...
def convert_post(msg_text, Api):
    ...
    for i in range(nml_len):
        ...
        count += 1
    new_urls = short_urls(urls, t_api=Api)
